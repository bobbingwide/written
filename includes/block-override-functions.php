 ...
 */
function written_maybe_override_block( $args, $blockname, $render_callback ) {
    $written_render_callback = 'written_' . $render_callback;
    if ( $blockname == $args['name'] && function_exists( $written_render_callback ) ) {
        if ( 'gutenberg_' . $render_callback == $args['render_callback'] ) {
            $args['render_callback'] = $written_render_callback;
        }
    }
    return $args;
}